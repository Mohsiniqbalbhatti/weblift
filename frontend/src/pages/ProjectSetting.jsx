...
function ProjectSetting() {
  ...
  const getProjectInfo = async () => {
    ...
    try {
      ...
      // Retry after a short delay if it's a new project
      setTimeout(() => {
        getProjectInfo();
      }, 2000);
    } finally {
  ...
  return (
    <>
      {load && (
        ...
        >
          <Loader />
          <p className="mt-3 text-center">Loading project...</p>
        </div>
      )}
      {!project && !load && (
        <div className="text-center mt-5">
          <h4>Project not found or loading...</h4>
          <button className="btn btn-primary mt-3" onClick={getProjectInfo}>
          ...
          {githubDeployment ? (
            <div className="col-12 card mt-3">
              <div className="d-flex justify-content-between flex-column flex-md-row">
                ...
                {deploymentAccess && (
                  <button
                    className="btn-main my-3"
                    onClick={triggerDeployement}
                  >
                    Trigger Deployment
                  </button>
                )}
